...
void deploy_hotspot(std::string *internet_interface, std::string *hotspot_interface, std::string *vpn_interface,  std::string *hotspot_ssid, std::string *hotspot_channel, std::string *hotspot_password);
...
int main(void)
{
    ...
    /*Deploy the hotspot.*/ 
    deploy_hotspot(&internet_interface, &hotspot_interface, &vpn_interface, &hotspot_ssid, &hotspot_channel, &hotspot_password);
...
void deploy_hotspot(std::string *internet_interface, std::string *hotspot_interface, std::string *vpn_interface, std::string *hotspot_ssid, std::string *hotspot_channel, std::string *hotspot_password)
{
    ...
    /*Setup firewall rules, start hostapd and dnsmasq WITHOUT a VPN.*/
    ...
    /*Setup firewall rules, start hostapd and dnsmasq WITH a VPN.*/
    // std::string deploy_hotspot = "";
    // deploy_hotspot += "killall hostapd > /dev/null 2>&1;ifconfig " + *hotspot_interface + " up 192.168.2.1 netmask 255.255.255.0;";
    // deploy_hotspot += "iptables -F;iptables -t nat -A POSTROUTING -o " + *vpn_interface + " -j MASQUERADE;";
    // deploy_hotspot += "iptables -A FORWARD -i " + *vpn_interface + " -o " + *hotspot_interface + " -m state --state RELATED,ESTABLISHED -j ACCEPT;";
    // deploy_hotspot += "iptables -A FORWARD -i " + *hotspot_interface + " -o " + *vpn_interface + " -j ACCEPT;";
    // deploy_hotspot += "sysctl -w net.ipv4.ip_forward=1 > /dev/null 2>&1;systemctl stop systemd-resolved;killall -9 dnsmasq > /dev/null 2>&1;";
    // deploy_hotspot +="hostapd hostapd.conf -B > /dev/null 2>&1;printf \"# Generated by NetworkManager\nsearch lan\nnameserver 1.1.1.1\nnameserver 1.0.0.1\" > /etc/resolv.conf;";
    // deploy_hotspot +="dnsmasq -C dnsmasq.conf";
    // system(deploy_hotspot.c_str());
